...
export function AdminSidebar() {
    ...
    const NavLinks = ({ isMobile = false }) => (
        <nav className={cn(
            ...
            (isSidebarCollapsed && !isMobile) ? "p-4" : "p-6"
        )}>
            {navItems.map((item) => {
                    ...
                    >
                        <Button
                            ...
                            className={cn(
                                ...
                                (isSidebarCollapsed && !isMobile) ? "justify-center px-0" : "justify-start px-6",
                                isActive
                        ...
                        >
                            ...
                            {(!isSidebarCollapsed || isMobile) && (
                                <span className={cn(
                                    "text-[10px] animate-in slide-in-from-left-2 duration-300 whitespace-normal text-left leading-normal",
                                )}>
                                    {item.name}
                                </span>
                            )}
        ...
        <>
            ...
            <div className="lg:hidden fixed top-0 left-0 right-0 h-16 bg-white border-b border-gray-100 z-50 flex items-center justify-between px-6">
                <Link href="/admin" className="flex items-center gap-3">
                    <LuxechoLogo size={24} />
                    <span className="text-xs font-black uppercase tracking-[0.4em]">Admin</span>
                </Link>
                <Sheet open={isMobileMenuOpen} onOpenChange={setIsMobileMenuOpen}>
                    <SheetTrigger asChild>
                        <Button variant="ghost" size="icon" className="rounded-none w-10 h-10 hover:bg-gray-50">
                    ...
                    <SheetContent side="left" className="p-0 w-[85vw] max-w-sm border-r-0 rounded-none hide-close-button bg-white flex flex-col">
                        ...
                        <div className="p-8 border-t border-gray-50">
                            <div className="flex items-center gap-3">
                                <div className="w-8 h-8 rounded-full bg-black text-white flex items-center justify-center text-[10px] font-black shrink-0">VK</div>
                                <div>
            ...
            <aside className={cn(
                ...
                {isSidebarCollapsed && (
                    <div className="p-4 border-t border-gray-50 flex justify-center shrink-0">
                        <Button
                            variant="ghost"
                            size="icon"
                            onClick={toggleSidebar}
                ...
                {!isSidebarCollapsed && (
                    <div className="p-8 border-t border-gray-50 animate-in fade-in duration-500 shrink-0">
                        <div className="flex items-center gap-3">
                            <div className="w-8 h-8 rounded-full bg-black text-white flex items-center justify-center text-[10px] font-black shrink-0">VK</div>
                            <div>
                                <p className="text-[10px] font-black uppercase tracking-widest text-black">Administrator</p>
